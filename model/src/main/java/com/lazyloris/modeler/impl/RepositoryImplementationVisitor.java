...
public class RepositoryImplementationVisitor extends FreemarkerAdaptor implements ModelVisitor {

	@Override
	public void visit(Model model) {
        addParameter("model", model);
        for (Module m : model.getModules()) {
            setPackageName(m.getName() + ".repository.impl");
            setOutputPath(getPackagePath());
            setFileName("RepositoryImpl.java");
